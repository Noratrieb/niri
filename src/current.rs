...
use rustix;
use rustix::io::{close, read, retry_on_intr, write};
use rustix::pipe::{PipeFlags, pipe_with};

pub fn do_spawn(command: &OsStr, mut process: Command) -> Option<Duration> {
    #[cfg(target_env = "gnu")]
    ...
    let (pipe_pid_read, pipe_pid_write) = pipe_with(PipeFlags::CLOEXEC)
        .map_err(|err| {
            println!("error creating a pipe to transfer child PID: {err:?}");
        })
        .ok()
        .unzip();
    ...
    let (pipe_wait_read, pipe_wait_write) = pipe_with(PipeFlags::CLOEXEC)
        .map_err(|err| {
            println!("error creating a pipe for child to wait on: {err:?}");
        })
        .ok()
        .unzip();
    ...
    let child = match process.spawn() {
        Ok(child) => child,
        Err(err) => {
            println!("error spawning {command:?}: {err:?}");
            return None;
        }
    };
        ...
        match read_all(pipe, &mut buf) {
            Ok(()) => {
                ...
                if let Err(err) = start_systemd_scope(command, child.id(), pid as u32) {
                    println!("error starting systemd scope for spawned command: {err:?}");
                }
                duration = Some(before.elapsed());
            }
            Err(err) => {
                println!("error reading child PID: {err:?}");
            }
        }
    }
...
fn start_systemd_scope(name: &OsStr, intermediate_pid: u32, child_pid: u32) -> anyhow::Result<()> {
    ...
    let mut scope_name = String::from("app-niri-perf-test-");
